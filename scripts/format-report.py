#!/usr/bin/env python3
"""
Format intelligence data into various report formats.
...
def format_person_markdown(person: dict) -> str:
    ...
    md = f"# Person Intelligence: {person.get('name', 'Unknown')}\n\n"
...
def format_company_markdown(company: dict) -> str:
    ...
    md = f"# Company Intelligence: {company.get('name', 'Unknown')}\n\n"
...
def format_to_csv(data: dict) -> str:
    """Convert intelligence data to CSV format."""
    output = StringIO()
...
def format_to_html(data: dict) -> str:
    """Convert intelligence data to HTML report."""
    html = """<!DOCTYPE html>
<html>
<head>
...
"""
    ...
    if data.get('export_metadata'):
        ...
        html += f"<p class='meta'>Generated: {meta.get('timestamp', '')} | Source: {meta.get('source', 'sales-recon')}</p>\n"

    # Persons
    if data.get('persons'):
        html += "<h2>Person Intelligence</h2>\n"
        for person in data['persons']:
    ...
    <p>📧 {person.get('email', 'N/A')} | 📞 {person.get('phone', 'N/A')} | 🔗 <a href="{person.get('linkedin_url', '#')}">{person.get('linkedin_url', 'N/A')}</a></p>
    <p>📍 {person.get('location', 'Unknown')}</p>
</div>\n"""

    # Companies
    if data.get('companies'):
        html += "<h2>Company Intelligence</h2>\n"
        for company in data['companies']:
            html += f"""<div class="card">
    ...
    <p>🌐 <a href="{company.get('website', '#')}">{company.get('website', 'N/A')}</a> | 📞 {company.get('headquarters', {}).get('phone', 'N/A')}</p>
"""
            if company.get('leadership'):
    ...
    html += """
    <p class="meta">Generated by Sales Recon Plugin</p>
</body>
</html>"""
